...
if (localStorage.length != 0) {
    ...
    if (data.length != 0){
        for (i in data){
            ...
            if(
                current_reminder['startHours'] == '' &&
                current_reminder['startMinutes'] == '' &&
                current_reminder['startSeconds'] == '' &&
                current_reminder['startMeredim'] == '' &&
                current_reminder['endHours'] == '' &&
                current_reminder['endMinutes'] == '' &&
                current_reminder['endSeconds'] == '' &&
                current_reminder['endMeredim'] == ''
            ){
                ending_date = new Date(current_reminder.endDate)
                today_date = new Date()
                timediffernce = ending_date.getTime() - today_date.getTime();
                remainingDays = Math.ceil(timediffernce / (1000 * 3600 * 24));
